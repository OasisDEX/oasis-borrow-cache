...
import { managerGiveTransformer, openCdpTransformer } from '../../../src/borrow/transformers/cdpManagerTransformer';
...
describe('CdpMigrationTransformer', () => {
    ...
    beforeEach(async () => {
        [services, txServices] = await createServices({ addresses: { MIGRATION: '0xc73e0383f3aff3215e6f04b0331d58cecf0ab849' } } as Partial<SpockConfig>)
    ...
    it('handles TRANSFER events', async () => {
        ...
        const allEvents = await getSQL(services.db, `SELECT * FROM vault.events e WHERE e.kind = 'TRANSFER';`);
        expect(allEvents).toEqual([
            {
                ...
                v_gem: null,
                w_dai: null,
                cdp_id: "0xddb108893104de4e1c6d0e47c42237db4e617acc",
