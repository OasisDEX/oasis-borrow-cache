...
function getEventsFromBlockRange(
...
function getCombinedMultiplyEvents(
  services: LocalServices,
  start: number,
  end: number,
): Promise<MultiplyDbEvent[]> {
  return services.tx.manyOrNone(
    `
    SELECT 
...
async function getEventsForUrnsBeforeBlock(
  services: LocalServices,
  blockId: number,
  urns: string[],
): Promise<Event[]> {
...
async function saveEventsToDb(services: LocalServices, events: EventToSave[]): Promise<void> {
...
export const multiplyHistoryTransformer: (
  ...
  return {
    ...
    transform: async (services, _logs) => {
      ...
      const multiplyEvents: MultiplyDbEvent[] = await getCombinedMultiplyEvents(
        services,
        minBlock,
        maxBlock,
      );
      ...
      const allEventsBeforeBatch: Event[] = await getEventsForUrnsBeforeBlock(
        services,
        minBlock,
        uniqueUrns,
      );
