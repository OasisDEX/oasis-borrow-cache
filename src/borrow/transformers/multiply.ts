...
const handleMultiply = async (params: Dictionary<any>, log: PersistedLog, services: LocalServices, dependencies: Dependencies) => {
  ...
  const ilk = await dependencies.getIlkForCdp(
    cdpId,
    services
  )

  const liquidationRatio = await dependencies.getLiquidationRatio(
    ilk,
    block.number,
    services,
  )
