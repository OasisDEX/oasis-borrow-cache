...
RETURNS TABLE (
  ...
  SELECT * FROM (SELECT
    ...
    FROM (
      SELECT * FROM oasis_market.log_take 
      WHERE 
        base_amt > 0.0000001 AND quote_amt > 0.0000001
        AND timestamp >= date_arg - INTERVAL '1' day 
        AND offer_id NOT IN (30563, 207720, 389406, 434810) -- filter out dummy trades from 16.09.2019
        AND offer_id NOT IN (343418, 506069) -- filter out dummy trades from 10.10.2019
    ) t
  ...
  WHERE
    base_gem=base_gem_arg AND quote_gem=quote_gem_arg 
    AND date >= date_arg
